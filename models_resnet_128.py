...
def generator_txt2img_resnet(input_z_txt, is_train=True):
    ...
    net = BatchNorm(act=tl.act.lrelu, is_train=is_train, gamma_init=gamma_init, name='g_h1_res/batch_norm')(net)
...
def discriminator_txt2img_resnet(input_images, t_txt, is_train=True, reuse=False):
    ...
    if t_txt is not None:
        ...
        net_txt = Tile([1, 8, 8, 1], name='d_txt/tile')(net_txt)
        ...
        #net_ho = Conv2d(1, (s16, s16), (s16, s16), act=tf.identity, padding='VALID', W_init=w_init, name='d_ho/conv2d')(net_h4)
        # 1 x 1 x 1
